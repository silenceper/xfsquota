...
func (p *ProjectQuota) ClearQuota(targetPath string) error {
	...
	// save to file
	if p.prjFile != nil {
		_ = p.prjFile.UpdateProjects(p.idPaths)
		_ = p.prjFile.UpdateProjIds(p.idNames)
	}
...
func (p *ProjectQuota) findOrCreateBackingDev(targetPath string) (*backingDev, error) {
	...
	if err != nil && strings.Contains(err.Error(), "inappropriate ioctl for device") {
		...
		err = unix.Unlink(backingFsBlockDev)
		if err != nil {
			klog.Errorf("unlink backing device failed: %v", err)
		}
...
func (p *ProjectQuota) findOrCreateSharedProjectId(targetPath, projName string) (quotaID, bool, error) {
	...
	isNewId = true
	projId, _, err := p.findOrCreateProjectId(targetPath, !projIdNoCreate, !persistToFile)
	if err != nil {
		return 0, false, err
	}
...
func (p *ProjectQuota) findOrCreateProjectId(targetPath string,
	...
	p.idNames[newId] = newId.IdName("xfsquota")
	// save to file
